...
class ControllerAddProduct extends Controller {
    ...
    public function __construct($registry) {
        ...
        if (empty($this->session->data['token'])) { $this->session->data['token'] = bin2hex(random_bytes(32)); }
        if (($this->request->server['REQUEST_METHOD'] ?? '') === 'POST') {
            $token = $this->request->post['token'] ?? $this->request->get['token'] ?? '';
            if (!is_string($token) || !hash_equals($this->session->data['token'], $token)) {
                http_response_code(403); exit('Обновите страницу и повторите действие.');
            }
        }
        $ids = isset($this->request->get['item_id']) ? array($this->request->get['item_id']) : array();
        if (isset($this->request->post['selected'])) {
            if (!is_array($this->request->post['selected'])) { http_response_code(400); exit; }
            $ids = array_merge($ids, $this->request->post['selected']);
        }
        $this->load->model('add/product');
        foreach ($ids as $id) {
            if (!is_scalar($id) || !$this->model_add_product->ownsProduct($id)) {
                http_response_code(403); exit('Товар недоступен.');
            }
        }
    }
	...
	public function index() {
		$this->load->language('add/product');
...
$data['acc'] = $this->load->controller('extension/module/seller');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('add/product', 'token=' . $this->session->data['token'] . $url, true));
		}
...
$data['acc'] = $this->load->controller('extension/module/seller');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_add_product->editProduct($this->request->get['item_id'], $this->request->post);
...
$data['acc'] = $this->load->controller('extension/module/seller');
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_add_product->deleteProduct($product_id);
			}
...
$data['acc'] = $this->load->controller('extension/module/seller');
		...
		if (isset($this->request->post['selected']) && $this->validateCopy()) {
			foreach ($this->request->post['selected'] as $product_id) {
				$this->model_add_product->copyProduct($product_id);
			}
...
$data['acc'] = $this->load->controller('extension/module/seller');
		...
		if (isset($filter_category)) {
			if ($filter_category>0) {
				$this->load->model('add/category');
				$category = $this->model_add_category->getCategory($filter_category);
				if ($category) {
		...
		if (isset($filter_manufacturer)) {
			if ($filter_manufacturer>0) {
				$this->load->model('add/manufacturer');
				$manufacturer = $this->model_add_manufacturer->getManufacturer($filter_manufacturer);
				if ($manufacturer) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('add/product', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('add/product/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['copy'] = $this->url->link('add/product/copy', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('add/product/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['enabled'] = $this->url->link('add/product/enable', 'token=' . $this->session->data['token'] . $url, true);
        $data['disabled'] = $this->url->link('add/product/disable', 'token=' . $this->session->data['token'] . $url, true);
		...
		$product_total = $this->model_add_product->getTotalProducts($filter_data);

		$results = $this->model_add_product->getProducts($filter_data);
		...
		foreach ($results as $result) {
			...
			$product_specials = $this->model_add_product->getProductSpecials($result['product_id']);
			...
			$data['products'][] = array(
				...
				'status'     => $result['status'] ? 'включен' : 'отключен',
				...
				'edit'       => $this->url->link('add/product/edit', 'token=' . $this->session->data['token'] . '&item_id=' . $result['product_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, true);
		$data['sort_model'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . '&sort=p.model' . $url, true);
		$data['sort_price'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, true);
		$data['sort_quantity'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, true);
		$data['sort_status'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, true);
		$data['sort_noindex'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . '&sort=p.noindex' . $url, true);
		$data['sort_order'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('add/product', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
...
$customer_info = $this->model_seller_seller->getseller($this->seller->getId());
$data['firstname'] = $customer_info['firstname'];
 
$data['easyphoto_form'] = $this->load->view('add/images', $data);
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('add/product_list', $data));
	}
...
$data['acc'] = $this->load->controller('extension/module/seller');
		...
		$data['text_city_select'] = $this->language->get('text_city_select');
		$data['text_currency_sar'] = $this->language->get('text_currency_sar');
		...
		if (!isset($this->request->get['item_id'])) {
			$data['action'] = $this->url->link('add/product/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('add/product/edit', 'token=' . $this->session->data['token'] . '&item_id=' . $this->request->get['item_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('add/product', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['item_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$product_info = $this->model_add_product->getProduct($this->request->get['item_id']);
		}
		...
		if (isset($this->request->post['product_description'])) {
			...
			$data['product_description'] = $this->model_add_product->getProductDescriptions($this->request->get['item_id']);
		} else {
		...
		// Delivery city of the product (stored in the existing product.location column)
		...
		if (isset($this->request->post['product_store'])) {
			...
			$data['product_store'] = $this->model_add_product->getProductStores($this->request->get['item_id']);
		} else {
...
if (isset($this->request->post['sort_order'])) {
	...
	if (!empty($product_info)) {
			$data['item_id'] = '_'.$product_info['product_id'];
		}   else {
			$data['item_id'] = '';
		}
//echo '----'.$data['item_id'];
...
$this->load->model('customer/customer');
		...
		if (isset($this->request->post['manufacturer'])) {
			...
			$manufacturer_info = $this->model_customer_customer->getCustomer($product_info['manufacturer_id']);

			if ($manufacturer_info) {
				$data['manufacturer'] = $manufacturer_info['firstname'].' '.$manufacturer_info['lastname'];
			} else {
				$data['manufacturer'] = '';
			}
		} else {
		...
		$categories = $this->model_add_category->getAllCategories();
		
		$data['categories'] = $this->model_add_category->getCategories($categories);
		
		if (isset($this->request->post['main_category_id'])) {
			$data['main_category_id'] = $this->request->post['main_category_id'];
		} elseif (isset($product_info)) {
			$data['main_category_id'] = $this->model_add_product->getProductMainCategoryId($this->request->get['item_id']);
		} else {
		...
		if (isset($this->request->post['product_category'])) {
			...
			$categories = $this->model_add_product->getProductCategories($this->request->get['item_id']);
		} else {
			$categories = array();
		}
		...
		foreach ($categories as $category_id) {
			$category_info = $this->model_add_category->getCategory($category_id);
...
$data['filters'] = array();
		...
		$results = $this->model_add_filter->getFilterGroups($filter_data);

		foreach ($results as $result) {
			$data['filters'][] = array(
				'filter_group_id' => $result['filter_group_id'],
				'name'            => $result['name'],
				'filters' => $this->model_add_filter->getFilterDescriptions($result['filter_group_id']),
				'sort_order'      => $result['sort_order'],
				'edit'            => $this->url->link('add/filter/edit', 'token=' . $this->session->data['token'] . '&filter_group_id=' . $result['filter_group_id'] . $url, true)
			);
		}
		...
		if (isset($this->request->post['product_filter'])) {
			...
			$filters = $this->model_add_product->getProductFilters($this->request->get['item_id']);
		} else {
			$filters = array();
		}
$data['prfil'] = $filters;
		...
		foreach ($filters as $filter_id) {
			$filter_info = $this->model_add_filter->getFilter($filter_id);
		...
		// Attributes
		$this->load->model('add/attribute');

		if (isset($this->request->post['product_attribute'])) {
			$product_attributes = $this->request->post['product_attribute'];
		} elseif (isset($this->request->get['item_id'])) {
			$product_attributes = $this->model_add_product->getProductAttributes($this->request->get['item_id']);
		} else {
		...
		foreach ($product_attributes as $product_attribute) {
			$attribute_info = $this->model_add_attribute->getAttribute($product_attribute['attribute_id']);
		...
		// Options
		$this->load->model('add/option');

		if (isset($this->request->post['product_option'])) {
			$product_options = $this->request->post['product_option'];
		} elseif (isset($this->request->get['item_id'])) {
			$product_options = $this->model_add_product->getProductOptions($this->request->get['item_id']);
		} else {
		...
		foreach ($data['product_options'] as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				if (!isset($data['option_values'][$product_option['option_id']])) {
					$data['option_values'][$product_option['option_id']] = $this->model_add_option->getOptionValues($product_option['option_id']);
				}
			}
		}
		...
		if (isset($this->request->post['product_discount'])) {
			...
			$product_discounts = $this->model_add_product->getProductDiscounts($this->request->get['item_id']);
		} else {
		...
		if (isset($this->request->post['product_special'])) {
			...
			$product_specials = $this->model_add_product->getProductSpecials($this->request->get['item_id']);
		} else {
...
$data['pricespeclial'] = '';
		foreach ($product_specials as $product_special) {
			$data['pricespeclial']= $product_special['price'];
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_add_product->getProductImages($this->request->get['item_id']);
		} else {
		...
		// Downloads
		$this->load->model('add/download');

		if (isset($this->request->post['product_download'])) {
			$product_downloads = $this->request->post['product_download'];
		} elseif (isset($this->request->get['item_id'])) {
			$product_downloads = $this->model_add_product->getProductDownloads($this->request->get['item_id']);
		} else {
		...
		foreach ($product_downloads as $download_id) {
			$download_info = $this->model_add_download->getDownload($download_id);
		...
		if (isset($this->request->post['product_related'])) {
			...
			$products = $this->model_add_product->getProductRelated($this->request->get['item_id']);
		} else {
			$products = array();
		}
		...
		foreach ($products as $product_id) {
			$related_info = $this->model_add_product->getProduct($product_id);
		...
		if (isset($this->request->post['product_related_article'])) {
			...
			$articles = $this->model_add_product->getArticleRelated($this->request->get['item_id']);
		} else {
			$articles = array();
		}
		...
		if (isset($this->request->post['product_reward'])) {
			...
			$data['product_reward'] = $this->model_add_product->getProductRewards($this->request->get['item_id']);
		} else {
		...
		if (isset($this->request->post['product_layout'])) {
			...
			$data['product_layout'] = $this->model_add_product->getProductLayouts($this->request->get['item_id']);
		} else {
		...
		if(isset($this->request->post['product_tab'])){
			...
			$data['product_tabs'] = $this->model_add_product->getProductTabbyProductID($this->request->get['item_id']);
		}else{
...
$data['bonustotal'] =  $this->model_seller_seller->getBonustotal($this->seller->getId());
$data['easyphoto_form'] = $this->load->view('add/images', $data);
		$this->load->model('add/layout');

		$data['layouts'] = $this->model_add_layout->getLayouts();
		
		$this->load->model('add/sticker');
		$data['stickers'] = $this->model_add_sticker->getStickersProduct();
		
		
		$this->load->model('add/benefit');
		$productbenefits = $this->model_add_benefit->getBenefits();
		...
		if (isset($this->request->post['product_benefits'])) {
			...
			$data['product_benefits'] = $this->model_add_product->getBenefits($this->request->get['item_id']);
		} else {
		...
		if (isset($this->request->post['product_stickers'])) {
			...
			$data['product_stickers'] = $this->model_add_sticker->getProductSticker($this->request->get['item_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('add/product_form', $data));
	}

	/**
	 * Cities a seller may place a product in (tech.md section 4.1).
	 * Keys are stored in product.location.
	 */
    ...
    protected function validateForm() {
        ...
        if (!empty($this->request->get['item_id'])) {
            ...
            foreach ($this->model_add_product->getProductImages($this->request->get['item_id']) as $image) { $existing[] = $image['image']; }
        }
	...
	public function enable() {
        ...
        if (isset($this->request->post['selected'])) {
            foreach ($this->request->post['selected'] as $product_id) {
                $this->model_add_product->editProductStatus($product_id, 1);
            }
            $this->session->data['success'] = $this->language->get('text_success');
            $url = '';
            if (isset($this->request->get['page'])) {
    ...
    public function disable() {
        ...
        if (isset($this->request->post['selected'])) {
            foreach ($this->request->post['selected'] as $product_id) {
                $this->model_add_product->editProductStatus($product_id, 0);
            }
            $this->session->data['success'] = $this->language->get('text_success');
            $url = '';
            if (isset($this->request->get['page'])) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			foreach ($results as $result) {
				...
				$product_options = $this->model_add_product->getProductOptions($result['product_id']);

				foreach ($product_options as $product_option) {
					$option_info = $this->model_add_option->getOption($product_option['option_id']);

					if ($option_info) {
						$product_option_value_data = array();

						foreach ($product_option['product_option_value'] as $product_option_value) {
							$option_value_info = $this->model_add_option->getOptionValue($product_option_value['option_value_id']);
