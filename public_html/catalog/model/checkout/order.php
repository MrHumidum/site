...
if (isset($total['prods'])) {
	foreach ($total['prods'] as $discon) {
if ($discon['product_id'] == $product['product_id']) {
$discount = $discon['discount'];
}

}
}
	}
$costtotal = $product['price']-$discount;
$bonus = ($product['price']-$discount)*((100-(float)$product['komis'])/100);

if (isset($this->session->data['coupons'])) {
foreach ($this->session->data['coupons'] as $cops) {
if ($product['product_id'] == $cops['product_id']) { $bonus = $bonus - $cops['descont'];
$costtotal = $costtotal-$cops['descont'];
 }
  if ($cops['product_id']==0) { $bonus = $bonus - $cops['descont'];
$costtotal = $costtotal-$cops['descont'];
   }  
}
}

$bonus *= (int)$product['quantity'];
$costtotal *= (int)$product['quantity'];
$this->db->query("INSERT INTO " . DB_PREFIX . "bonus SET order_id = '" . (int)$order_id . "', summ = '" . (float)$bonus . "', user_id = '" . (int)$product['manufacturer_id'] . "', prname = '" . $this->db->escape($product['name']) . "', product_id = '" . (int)$product['product_id'] . "' , cost='".$product['price']."', costtotal='".$costtotal."', status=0 , date = NOW()");
//$this->db->query("UPDATE " . DB_PREFIX . "customer bonus = bonus+'".$bonus."'");
	...
	public function ChangeMail($order_id, $newmail) {
...
$this->db->query("UPDATE `" . DB_PREFIX . "order` SET email = '" . $newmail . "'  WHERE order_id = '" . (int)$order_id . "'");
			...
			$item= $order_product['name'].' Cсылка '.$order_product['model'];
				}
    ...
    public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
        ...
        if (empty($lock->row['acquired'])) { return false; }
        try {
            $order = $this->getOrder($order_id);
            if (!$order) { return false; }
            if ($order['payment_code'] === 'bank_transfer') {
                $current = (int)$order['order_status_id'];
                $next = (int)$order_status_id;
                if ($current === $next) { return true; }
                $allowed = array(0 => array(4), 4 => array(5, 3), 5 => array(1, 3), 1 => array(3), 2 => array(3), 3 => array());
                if (!isset($allowed[$current]) || !in_array($next, $allowed[$current], true)) { return false; }
            }
            $this->applyOrderHistory($order_id, $order_status_id, $comment, $notify, $override);
            return true;
        } finally {
    ...
    private function applyOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		$order_info = $this->getOrder($order_id);
	...
	$data['myname'] = $order_info['firstname'];
				$data['payment_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
