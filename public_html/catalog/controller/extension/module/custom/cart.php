		...
		if (isset($setting['status']) && (bool)$setting['status'] === true) {
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'thumb'     => $image,
					'oldprice' => $product['oldprice'] ? $this->currency->format($this->tax->calculate($product['oldprice'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']) : '',
					'name'      => $product['name'],
					'model'     => $product['model'],
			...
			if (empty($setting)){
				...
				$data['setting'] = $setting['custom_cart'];
				$this->response->setOutput($this->load->view('extension/module/custom/cart', $data));
			} else {
