...
$summ = $product['total']*(100-$product['komis'])/100;
echo $summ;
$this->db->query("UPDATE " . DB_PREFIX . "customer SET bonus = bonus + '".$summ."' ");
 ...
 $this->db->query("DELETE FROM " . DB_PREFIX . "bonus  WHERE order_id = '" . (int) $order_id . "'");
        // Products
        foreach ($data['products'] as $product) {

$summ = $product['total']*(100-$product['komis'])/100;
//$this->db->query("UPDATE " . DB_PREFIX . "customer SET bonus = bonus + '".$summ."' ");
 $this->db->query("INSERT INTO " . DB_PREFIX . "bonus SET order_id = '" . (int) $order_id . "', product_id = '" . (int) $product['product_id'] . "',  prname = '" . $this->db->escape($product['name']) . "', user_id = '" . $this->db->escape($product['manufacturer_id']) . "', summ = '" . (float) $summ . "'");
