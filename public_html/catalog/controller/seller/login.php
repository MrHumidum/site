		...
		if (!empty($this->request->get['token'])) {
			...
			if ($seller_info && $this->seller->login($seller_info['email'], '', true)) {
				...
				if ($this->config->get('config_tax_seller') == 'payment') {
					$this->session->data['payment_address'] = $this->model_account_address->getAddress($this->seller->getAddressId());
				}

				if ($this->config->get('config_tax_seller') == 'shipping') {
					$this->session->data['shipping_address'] = $this->model_account_address->getAddress($this->seller->getAddressId());
				}

				$this->response->redirect($this->url->link('seller/account', '', true));
			}
		}
...
$this->session->data['token'] = token(32);
			...
			// Add to activity log
			if ($this->config->get('config_seller_activity')) {
			...
			if (isset($this->request->post['redirect']) && $this->request->post['redirect'] != $this->url->link('seller/logout', '', true) && (strpos($this->request->post['redirect'], $this->config->get('config_url')) !== false || strpos($this->request->post['redirect'], $this->config->get('config_ssl')) !== false)) {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('seller/login', '', true)
		);
	...
	protected function validate() {
		...
		$login_info = $this->model_seller_seller->getLoginAttempts($this->request->post['email']);

		if ($login_info && ($login_info['total'] >= $this->config->get('config_login_attempts')) && strtotime('-1 hour') < strtotime($login_info['date_modified'])) {
			$this->error['warning'] = $this->language->get('error_attempts');
		}

		// Check if seller has been approved.
		...
		if (!$this->error) {
			if (!$this->seller->login($this->request->post['email'], $this->request->post['password'])) {
				...
				$this->model_seller_seller->addLoginAttempt($this->request->post['email']);
			} else {
				$this->model_seller_seller->deleteLoginAttempts($this->request->post['email']);
			}
		}
