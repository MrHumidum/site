...
foreach ($categories as $category_id) {
	$data['back'] = $this->url->link('product/category', 'path=' . $category_id);
}
$data['incart'] = false;
foreach ($this->cart->getProducts() as $ppro) {
            if ($ppro['product_id'] == $product_id) {
$data['incart'] = true;
            }
        }
...
$avtor_info = $this->model_catalog_product->getCustomer($product_info['manufacturer_id']);
 $data['avtor_href']= $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $product_info['manufacturer_id']);
if ($avtor_info) {
$data['avtor_name'] = $avtor_info['firstname'];
$data['avtor_image'] = $this->model_tool_image->resize($avtor_info['image'], 100,100);
}
else { $data['avtor_name'] = false;}
			...
			$data['description2'] = html_entity_decode($product_info['description_mini'], ENT_QUOTES, 'UTF-8');
			...
			if ($product_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($product_info['image'], 890,890);
			} else {
				$data['thumb'] = '';
			}
$check_prodano = $this->model_catalog_product->getProductPurchased($this->request->get['product_id']);
if ($check_prodano['quantity'] != false) {
				$data['prodano'] = $check_prodano['quantity'];
			} else {
				$data['prodano'] = '0';
			}
			$data['images'] = array();
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'], 890,890) 
				);
			}
...
$pridd = $product_info['product_id'];
$categories = $this->model_catalog_product->getProductCategories($pridd);
...
$filter_data = array(
    'filter_manufacturer_id' => 4, // или 'filter_seller_id' если у вас используется другое поле
    'start'                  => 0,
    'limit'                  => 300  // получаем все товары продавца (его у него 300)
);
$results = $this->model_catalog_product->getProducts($filter_data);

// Перемешиваем массив товаров, чтобы выводить случайные позиции
shuffle($results);

// Оставляем только 4 товара для показа
