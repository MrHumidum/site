  ...
  <div class="container">
    <div class="category_wrap">
      <div class="c_c_head mob">
        ...
        <p>Товаров в категории: <span><span class="totalcountmob"><? echo $totalcount;?></span> шт.</span></p>
      </div>
...
<div class="category_card_box">
        <div class="c_c_head desk">
          <h2 class="title c_c_title"><?php echo $heading_title; ?></h2>
          <p>Товаров в категории: <span ><span class="totalcount"><? echo $totalcount;?></span> шт.</span></p>
        </div>
  <?php if ($categories) { ?>
  <nav class="category-image-grid" aria-label="Подкатегории">
    <?php foreach ($categories as $category) { ?>
    <a class="category-image-card" href="<?php echo $category['href']; ?>">
      <img src="<?php echo htmlspecialchars($category['image'], ENT_QUOTES, 'UTF-8'); ?>" alt="" width="160" height="160" loading="lazy" />
...
<?php if (!$product['special']) { ?>
          ...
          <?php } ?>
                  ...
                  <button class="card_btn addcart"  onclick="cart.add('<?php echo $product['product_id']; ?>');return false;">
                    <svg><use xlink:href="#card"></use></svg>
                  </button>
                  <button class="card_btn removecart"  onclick="cart.removep('<?php echo $product['product_id']; ?>');return false;">
...
<p class="showit"><? echo $results;?></p> </div>
<?php } ?> 
