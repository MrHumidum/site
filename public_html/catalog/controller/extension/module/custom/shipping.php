...
class ControllerExtensionModuleCustomShipping extends Controller {
	public function index($setting) {
		...
		if (isset($setting['status']) && (bool)$setting['status'] === true && $this->cart->hasShipping()) {
			...
			// Delivery is limited to Mecca and Medina (tech.md section 4.1).
			...
			if (isset($this->session->data['shipping_address']['city'])) {
				...
				$data['city'] = $this->getSelectedCity();
			}
	...
	/**
	 * Cities the marketplace delivers to. Values are stored in the order address.
	 */
	...
	/**
	 * City name chosen in the header selector, used as the checkout default.
	 */
	public function getSelectedCity() {
		$cities = $this->getCities();

		if (isset($this->session->data['city']) && isset($cities[$this->session->data['city']])) {
			return $cities[$this->session->data['city']];
		}
			...
			if ( $this->customer->isLogged() ) {
				...
				if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'existing') {
					...
					if (!$json) {
                        ...
                        if (!$this->model_marketplace_location->isDeliverable($address)) {
                            $json['error']['warning'] = 'Доставка доступна только в Мекке и Медине, Саудовская Аравия.';
                            unset($this->session->data['shipping_address'], $this->session->data['shipping_method']);
                        } else {
	...
	private function getAddress($fields, $post, $shipping_code){
		...
		foreach($fields as $field){
			...
			$name = $field['name'];
			$value = isset($post[$name]) && is_scalar($post[$name]) ? $post[$name] : '';
		...
		// The city must be one of the two supported cities, whatever the client sent.
		if (empty($address['city']) || !in_array($address['city'], $this->getCities(), true)) {
