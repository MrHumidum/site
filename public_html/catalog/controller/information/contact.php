...
$mess = "Имя: ".$this->request->post['username']."<br>Email: ".$this->request->post['useremail']."<br>Телефон: ".$this->request->post['userphone']."<br>Сообщение: ".$this->request->post['usermess'];
$spam=0;
$spamlist = array('купить', 'реклама', 'ссылка');
foreach($slova as $item)
    {
        if (preg_match("/$item/",$text)) { $span=1;}
    }
if ($spam <1){
			...
			$mail->setSubject(html_entity_decode(sprintf('Проблема с заказом (%s)', $this->request->post['username']), ENT_QUOTES, 'UTF-8'));
			$mail->setText($mess);
			$mail->send();
}
