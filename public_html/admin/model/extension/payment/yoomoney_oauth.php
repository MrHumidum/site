...
class ModelExtensionPaymentYoomoneyOauth extends Model
{
    ...
    {
        $this->getKassaLog()->sendHeka(array('oauth.get-token.init'));
        $data = array(
            'state' => $this->getOauthState()
        );
        ...
        try {
            ...
            $this->log('error', 'Failed to get OAuth token: ' . $e->getMessage());
            $this->getKassaLog()->sendHeka(array('oauth.get-token.fail'));
        ...
        try {
            $this->getKassaLog()->sendHeka(array('webhooks.subscribe.init'));
            $this->subscribe();
            $this->getKassaLog()->sendHeka(array('webhooks.subscribe.success'));
        } catch (Exception $e) {
            $this->log('error', $e->getMessage());
            $this->getKassaLog()->sendHeka(array('webhooks.subscribe.fail', 'oauth.get-token.fail'));
            return array('error' => 'Failed to get make webhooks');
        }

        try {
            $this->getKassaLog()->sendHeka(array('oauth.get-shop.init'));
            $this->saveShopInfo();
            $this->getKassaLog()->sendHeka(array('oauth.get-shop.success'));
        } catch (Exception $e) {
            $this->log('error', $e->getMessage());
            $this->getKassaLog()->sendHeka(array('oauth.get-shop.fail', 'oauth.get-token.fail'));
            return array('error' => 'Failed to get shop info');
        }

        $this->getKassaLog()->sendHeka(array('oauth.get-token.success'));
        return array('success' => true);
    }

    /**
     * Формирует параметры, инициирует отправку, получает результат запроса на отзыв токена
     *
    ...
    {
        $this->getKassaLog()->sendHeka(array('token.revoke.init'));
        $data = array(
            'state' => $this->getOauthState(),
            'token' => $token,
            'cms' => self::CMS_NAME
        ...
        if (!isset($data['success'])) {
            ...
            $this->getKassaLog()->sendHeka(array('token.revoke.fail'));
        } else {
            $this->getKassaLog()->sendHeka(array('token.revoke.success'));
        }
    }
