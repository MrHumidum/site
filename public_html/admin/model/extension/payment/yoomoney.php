...
class ModelExtensionPaymentYoomoney extends Model
{
    ...
    private $repository = 'cms/repos/cms-opencart2';
    ...
    {
        $this->getKassaLog()->sendHeka(array('module.install.init'));
        ...
        try {
            ...
            ');
            $this->getKassaLog()->sendHeka(array('module.install.success'));
        } catch (Exception $e) {
            $this->getKassaLog()->sendHeka(array('module.install.fail'));
        }
    }

    public function uninstall()
    {
        $this->getKassaLog()->sendHeka(array('module.uninstall.init'));
        $this->log('info', 'uninstall yoomoney module');
        $this->getKassaLog()->sendHeka(array('module.uninstall.success'));
    }
    ...
    {
        $this->getKassaLog()->sendHeka(array('payment-status.change.init'));
        ...
        $this->db->query($sql);
        $this->getKassaLog()->sendHeka(array('payment-status.change.success'));
    }
    ...
    {
        try {
            if ($refund = $this->getClient()->getRefundInfo($refundId)) {
    ...
    public function getKassaModel()
    {
        ...
        $this->kassaModel->setApiClient($apiClient);
    ...
    {
        $this->getKassaLog()->sendHeka(array('refund.create.init'));
        try {
            ...
            $request = $builder->build();
            $this->getKassaLog()->sendHeka(array('refund.create.success'));
        } catch (Exception $e) {
            $this->getKassaLog()->sendAlertLog('Failed to create refund', array(
                'methodid' => 'POST/refundPayment',
                'exception' => $e,
            ), array('refund.create.fail'));
            $this->log('error', 'Failed to create refund: '.$e->getMessage());

            return null;
        }

        // todo: наверное нужен чек

        try {
            $this->getKassaLog()->sendHeka(array('refund.request.init'));
            $response = $this->getClient()->createRefund($request);
            $this->getKassaLog()->sendHeka(array('refund.request.success'));
        } catch (Exception $e) {
            $this->getKassaLog()->sendAlertLog('Failed to create refund', array(
                'methodid' => 'POST/refundPayment',
                'exception' => $e,
            ), array('refund.request.fail'));
            $this->log('error', 'Failed to create refund: '.$e->getMessage());
    ...
    {
        return $this->getKassaModel()->getApiClient();
    }
    ...
    {
        if (!class_exists('BitbucketConnector')) {
