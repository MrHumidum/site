...
class ControllerSaleOrder extends Controller {
	// Quick payment actions, see the tech.md v1.1.0 DB contract (section 7.3).
...
foreach ($results as $result) {
	$data['customers'][] = array(
		...
		'total'         => round($result['total']).' P',
		'date_added'     => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
	);
}

      $output = '';
      $customer_export = 'nulled_export_' . (string)(date('Y-m-d-Hi')) . '.csv';
...
fputs($handle, chr(0xEF) . chr(0xBB) . chr(0xBF)); // BOM
	...
	foreach (  $data['customers'] as $fields) {
...
$this->response->setOutput($output);
	...
	public function info() {
		...
		if ($order_info) {
			...
			// Payment receipt uploaded by the customer (tech.md section 4.4)
			$data['text_receipt'] = $this->language->get('text_receipt');
			$data['text_receipt_none'] = $this->language->get('text_receipt_none');
			$data['text_receipt_open'] = $this->language->get('text_receipt_open');
			$data['button_payment_approve'] = $this->language->get('button_payment_approve');
			$data['button_payment_reject'] = $this->language->get('button_payment_reject');

			$data['can_verify_payment'] = $this->user->hasPermission('modify', 'sale/order') && (int)$order_info['order_status_id'] === 5;
            $data['receipt_file'] = isset($order_info['receipt_file']) ? $order_info['receipt_file'] : '';
			$data['receipt_is_image'] = $data['receipt_file'] && preg_match('/\.(jpg|jpeg|png)$/i', $data['receipt_file']);

			if ($data['receipt_file']) {
				$data['receipt_url'] = $this->url->link('sale/order/receipt', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$order_id, true);
			} else {
				$data['receipt_url'] = '';
			}

			$data['payment_approve_status_id'] = self::STATUS_PROCESSING;
			$data['payment_reject_status_id'] = self::STATUS_CANCELLED;
	...
	/**
	 * Streams the payment receipt stored in DIR_UPLOAD for the given order.
	 */
	public function receipt() {
		...
		// basename() keeps a crafted value from escaping the upload directory.
		$filename = ($order_info && !empty($order_info['receipt_file'])) ? basename($order_info['receipt_file']) : '';

		if (!$filename || !preg_match('/^receipt_' . $order_id . '_[a-zA-Z0-9]+\.(jpg|jpeg|png|pdf)$/', $filename) || !is_file(DIR_UPLOAD . $filename)) {
			$this->response->addHeader($this->request->server['SERVER_PROTOCOL'] . ' 404 Not Found');
			$this->response->setOutput($this->language->get('error_receipt_missing'));
