...
class ModelAccountReturn extends Model {
	public function addReturn($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "vyvod` SET user_id = '" . (int)$this->customer->getId() . "', bank = '" . $this->db->escape($data['bank']) . "', summ = '" . $this->db->escape($data['summ']) . "', rek = '" . $this->db->escape($data['rek']) . "', status = '0', date = NOW()");
