 ...
 */
class ControllerExtensionModuleBrainyFilterAdd extends Controller {
    private $_data = array();
    private $_currency;
  ...
  public function eventEditProduct($route, $args, $output)
    {
     ...
     $this->load->model('extension/module/brainyfilteradd');
        $productId = $args[0];
       $this->model_extension_module_brainyfilteradd->cacheProductProperties($productId);
    }
	...
	public function index($moduleSettings) 
    {
        ...
        if (isset($this->request->get['route']) && $this->request->get['route'] === 'extension/module/brainyfilteradd/ajaxfilter') {
            return;
        }
		...
		$this->language->load('extension/module/brainyfilteradd');
		...
		$this->load->model('extension/module/brainyfilteradd');
        $model = new ModelExtensionModuleBrainyFilterAdd($this->registry);
        ...
        $postponedCount = $basicSettings['global']['postponed_count'] && empty($conditions->filters)
                && (!isset($conditions->manufacturer) || array($conditions->manufacturer) === 2);
...
//            }
        ...
        if (!isset($this->request->get['route']) 
                || ($this->request->get['route'] !== 'product/category' 
                 ...
                 && $this->request->get['route'] !== 'extension/module/brainyfilteradd/filter')) {
            $this->_data['redirectToUrl'] = $this->url->link('extension/module/brainyfilteradd/filter', (isset($this->request->get['path'])) ? 'path=' . $this->request->get['path'] : '');
        } else {
        ...
        $this->_data['manufacturerId'] = isset($this->request->get['autor_id']) ? $this->request->get['autor_id'] : '';
        ...
        if (version_compare(VERSION, '2.2.0.0', '>=')) {
            return $this->load->view('extension/module/brainyfilteradd', $this->_data);
        } else {
