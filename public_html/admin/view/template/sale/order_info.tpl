        ...
        <div class="panel panel-default">
          ...
          <table class="table">
            ...
            <tr>
              ...
              <td>
                <input type="text" name="mailuser" id="newmal" class="form-control" value="<?php echo $email; ?>">
                 
 <button id="button-mail" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"> Сохранить</button>
              </td>
            </tr>
            <tr>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title"><i class="fa fa-file-image-o"></i> <?php echo $text_receipt; ?></h3>
      </div>
      <div class="panel-body">
        <?php if ($receipt_file) { ?>
        <?php if ($receipt_is_image) { ?>
        <p><a href="<?php echo $receipt_url; ?>" target="_blank"><img src="<?php echo $receipt_url; ?>" alt="<?php echo $text_receipt; ?>" class="img-thumbnail" style="max-width: 320px;" /></a></p>
        <?php } ?>
        <p><a href="<?php echo $receipt_url; ?>" target="_blank" class="btn btn-default"><i class="fa fa-external-link"></i> <?php echo $text_receipt_open; ?></a></p>
        <div class="btn-group">
          <button type="button" id="button-payment-approve" <?php if (!$can_verify_payment) { echo 'disabled'; } ?> data-status="<?php echo $payment_approve_status_id; ?>" class="btn btn-success"><i class="fa fa-check-circle"></i> <?php echo $button_payment_approve; ?></button>
          <button type="button" id="button-payment-reject" <?php if (!$can_verify_payment) { echo 'disabled'; } ?> data-status="<?php echo $payment_reject_status_id; ?>" class="btn btn-danger"><i class="fa fa-times-circle"></i> <?php echo $button_payment_reject; ?></button>
        </div>
        <?php } else { ?>
        <p class="text-muted"><?php echo $text_receipt_none; ?></p>
        <?php } ?>
      </div>
    </div>
    <div class="panel panel-default">
...
$('#button-mail').on('click', function() {
  ...
  $.ajax({
    url: '<?php echo $catalog; ?>index.php?route=api/order/changemail&token=' + token + '&store_id=<?php echo $store_id; ?>&order_id=<?php echo $order_id; ?>',
    type: 'post',
    dataType: 'json',
    data: 'newmail=' + encodeURIComponent($('#newmal').val()) + '&order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&override=' + ($('input[name=\'override\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()),
    beforeSend: function() {
      $('#button-mail').button('loading');
    },
    complete: function() {
      $('#button-mail').button('reset');
    },
    success: function(json) {
      $('.alert').remove();
      ...
      if (json['success']) {
        alert('Товар отправлен на новый email');
      }
    },
    error: function(xhr, ajaxOptions, thrownError) {
      alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
// Quick payment decision: set the status and reuse the history submit.
$('#button-payment-approve, #button-payment-reject').on('click', function() {
	$('select[name=\'order_status_id\']').val($(this).data('status'));
