...
class ControllerExtensionPaymentYoomoneyB2bSberbank extends ControllerExtensionPaymentYoomoney
{
    ...
    public function createPayment($orderId)
    {
        ...
        $this->getModel()->getKassaLog()->sendHeka(array('payment.create.init'));
        ...
        try {
            ...
            try {
                ...
                $this->getModel()->getKassaLog()->sendAlertLog($this->language->get('b2b_tax_rates_error'), array(
            ...
            $this->getModel()->getKassaLog()->sendHeka(array('payment.create.success'));
        } catch (InvalidArgumentException $e) {
            $this->getModel()->log('error', 'Failed to create create payment request: '.$e->getMessage());
            $this->getModel()->getKassaLog()->sendAlertLog('Failed to create create payment request', array(
                'methodid' => 'POST/createPayment',
                'exception' => $e,
            ), array('payment.create.fail'));
            return null;
        ...
        try {
            $this->getModel()->getKassaLog()->sendHeka(array('payment.request.init'));
            $payment = $this->getModel()->getClient()->createPayment($request);
            $this->getModel()->getKassaLog()->sendHeka(array('payment.request.success'));
        } catch (Exception $e) {
            $this->getModel()->log('error', 'Failed to create payment: '.$e->getMessage());
            $this->getModel()->getKassaLog()->sendAlertLog('Failed to create payment', array(
                'methodid' => 'POST/createPayment',
                'exception' => $e,
