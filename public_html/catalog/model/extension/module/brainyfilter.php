...
final class ModelExtensionModuleBrainyFilter extends Model 
{
    ...
    public function __construct($registry) {
        ...
        if (isset($this->request->get['autor_id']) && !empty($this->request->get['autor_id'])) {
            $this->conditions->manufacturer = (int) $this->request->get['autor_id'];
            $this->conditions->filters['m0'][0] = (int) $this->request->get['autor_id'];
            $this->aggregate['m0'] = array();
            $this->aggregate['m0'][0] = (int) $this->request->get['autor_id'];
        }
    ...
    public function setData($data = array())
    {
        ...
        if (empty($this->conditions->filters['m0'])) {
            ...
            } elseif (isset($this->request->get['autor_id']) 
                    && !empty($this->request->get['autor_id'])) {
                $this->conditions->filters['m0'][0] = (int) $this->request->get['autor_id'];
                $this->aggregate['m0'] = array();
                $this->aggregate['m0'][0] = (int) $this->request->get['autor_id'];
            }
    ...
    public function prepareQueryForCategory()
    {
        ...
        {
			case 'random' : {
                $sql->order(array(" RAND() {$this->order}"));
                break;
            }
            ...
            case 'p.ean' : {
                $sql->select(array('pp.ean'))->order(array("pp.ean {$this->order}"))
    ...
    public function getFilters()
    {
		...
		//$sql->where('p.manufacturer_id = 2', array($this->conditions->manufacturer));
        $query = $this->db->query($sql);
