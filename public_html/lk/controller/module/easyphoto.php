...
class ControllerModuleEasyPhoto extends Controller {
	...
	public function install() {
		...
		// 	$this->model_setting_setting->editSettingValue('easyphoto', "easyphoto_key", $response['content']);
		// }
	}
	...
	public function index() {
		...
		// if($response['status'] && $response['content'] && empty($data['easyphoto_key'])){
		// 	$this->model_setting_setting->editSettingValue('easyphoto', "easyphoto_key", $response['content']);
		// 	$data['easyphoto_key'] = $response['content'];
		// }
		$data['easyphoto_key'] = 'nulled'; // $a?$data['easyphoto_key']:false;
	...
	public function getForm($product_images) {
		...
		return $this->load->view('module/easyphoto_form.tpl', $data);
	}

	public function upload() {
	echo $this->getDirectory(1);
	...
	public function resize_rename($from_model = array()) {
		...
		if(isset($this->request->get['product_id']) or isset($from_model['product_id'])){
			...
			//новое название фото
			if($name_from && $this->config->get('easyphoto_name')){ //если есть имя товара в том языке + включена опция - фото из названия
