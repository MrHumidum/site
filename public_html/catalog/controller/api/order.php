...
public function changemail() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				$this->model_checkout_order->ChangeMail($order_id, $this->request->post['newmail']);
...
//	
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				if ($this->model_checkout_order->addOrderHistory($order_id, $this->request->post['order_status_id'], $this->request->post['comment'], $this->request->post['notify'], $this->request->post['override'])) {
                    $json['success'] = $this->language->get('text_success');
                } else {
                    $json['error'] = 'Недопустимый переход статуса. Обновите заказ.';
                }
			} else {
				$json['error'] = $this->language->get('error_not_found');
			}
