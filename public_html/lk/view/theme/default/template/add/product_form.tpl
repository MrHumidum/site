      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          <ul class="nav nav-tabs hidden">
            <li class="active"><a href="#tab-general" data-toggle="tab">Описание</a></li>
<li><a href="#tab-easyphoto" data-toggle="tab">Изображения</a></li>
              ...
              <div class="tab-content">
                ...
                <div class="tab-pane active" id="language<?php echo $language['language_id']; ?>">
                  <div class="form-group required">
                    <label class="col-sm-2 control-label" for="input-name<?php echo $language['language_id']; ?>">Название игры</label>
                    <div class="col-sm-10">
                      <input type="text" name="product_description[<?php echo $language['language_id']; ?>][name]" value="<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['name'] : ''; ?>" placeholder="Название игры" id="input-name<?php echo $language['language_id']; ?>" class="form-control" />
          ...
          <div class="form-group">
                    <label class="col-sm-2 control-label" for="input-description_mini<?php echo $language['language_id']; ?>">Комплект игры</label>
                    <div class="col-sm-10">
       ...
       <div class="form-group required">
                <label class="col-sm-2 control-label" for="input-model">Ссылка на файл</label>
                <div class="col-sm-10">
                  <input type="text" name="model" value="<?php echo $model; ?>" placeholder="ссылка для скачивания" id="input-model" class="form-control" />
                  <?php if ($error_model) { ?>
...
<div class="form-group">
                ...
                <div class="col-sm-10">
<input type="text" name="pricespeclial" value="<?php echo $pricespeclial; ?>" placeholder="цена по акции" id="input-price" class="form-control" />
                </div>
              </div>        
        
       <div class="form-group">
                <label class="col-sm-2 control-label" for="input-ean"><span data-toggle="tooltip" title="порядок сортировки">Порядок сортировки</span></label>
                <div class="col-sm-10">
                  <input type="text" name="ean" value="<?php echo $ean; ?>" placeholder="Порядок сортировки" id="input-ean" class="form-control" />
                </div>
              </div>
...
<div class="form-group">
                <label class="col-sm-2 control-label" for="input-upc"><span data-toggle="tooltip" >Ютуб ссылка</span></label>
                <div class="col-sm-10">
                  <input type="text" name="upc" value="<?php echo $upc; ?>" placeholder="Ютуб ссылка" id="input-upc" class="form-control" />
                </div>
              </div>
        
<div class="form-group hidden">
                <label class="col-sm-2 control-label" for="input-manufacturer"><span data-toggle="tooltip" title="<?php echo $help_manufacturer; ?>">Продавец</span></label>
                <div class="col-sm-10">
...
<div class="selectfilter">
 <?php foreach ($filters as $filtergr) { ?>
<p><b><? echo $filtergr['name'];?></b></p>
<div class="filerlist">
  <?php foreach ($filtergr['filters'] as $product_filter1) { ?>
 
<label> <input type="checkbox" name="product_filter[]" value="<?php echo $product_filter1['filter_id']; ?>" <?if (in_array($product_filter1['filter_id'], $prfil)) {?> checked <? } ?>/><?php echo $product_filter1['filter_description'][1]['name']; ?></label>
 ...
 <? } ?>
  ...
  $(this).toggleClass('checked');
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <input type="text" name="quantity" value="100000" placeholder="<?php echo $entry_quantity; ?>" id="input-quantity" class="form-control" />
                </div>
              </div>
              <div class="form-group">
      ...
      <div class="tab-pane" id="tab-extra">
          <div class="row">
		   ...
		   <div class="pull-left">
        <button type="submit" form="form-product" data-toggle="tooltip" title="<?php echo $button_save; ?>" class="add_game_btn">Сохранить</button>
        </div>
        </form>
      </div>
    </div>
...
$('input[name=\'category\']').autocomplete({
  'source': function(request, response) {
    $.ajax({
      url: 'index.php?route=add/category/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
      dataType: 'json',
      success: function(json) {
        response($.map(json, function(item) {
          return {
...
$('input[name=\'filter\']').autocomplete({
  'source': function(request, response) {
    $.ajax({
      url: 'index.php?route=add/filter/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
      dataType: 'json',
      success: function(json) {
        response($.map(json, function(item) {
          return {
...
function setTranslit(source, dest, rewrite) {
	...
	var itemid="<? echo $item_id;?>";
	if (name != undefined) {
		$("input[name='"+source+"']").change(function(){
			var name = $("input[name='"+source+"']").val();
			var key = $("input[name='"+dest+"']").val();
			if ((key == '')||(rewrite))
				$("input[name='"+dest+"']").val(ru2en.translit(name)+itemid);
		});
	}
}
...
<style>
.filerlist label { margin-right: 10px; }
.filerlist label input { margin-right: 3px; }
</style>
