...
foreach ($results as $result) {
	$data['sellers'][] = array(
      ...
      $output = '';
      $seller_export = 'seller_export_' . (string)(date('Y-m-d-Hi')) . '.csv';

$tmp = DIR_SYSTEM . 'storage/logs/';
$uid = uniqid();
$tmp_dir = $tmp . '/' . $uid . '/';
$file = $tmp . '/' . $uid.'.csv';
...
fputs($handle, chr(0xEF) . chr(0xBB) . chr(0xBF)); // BOM
	...
	foreach (  $data['sellers'] as $fields) {
...
$this->response->addheader('Content-Disposition: attachment; filename='.$seller_export);
...
$this->response->setOutput($output);
	...
	public function aprov() {
		...
		$this->load->model('seller/seller');
			...
			$this->model_seller_seller->approvevyvod($this->request->get['vyvodid']);

			

			$this->response->redirect($this->url->link('seller/seller/getListVyvod', 'token=' . $this->session->data['token'], true));
...
$data['vyvod'] = array();
		...
		$results = $this->model_seller_seller->getVyvods($filter_data);

		foreach ($results as $result) {
		
if ($result['status']>0) { $stat = '<span class="green">Выполнено</span>';} else { $stat = '<span class="red">В ожидании</span>';}
			...
			$data['vyvod'][] = array(
				...
				'st'          => $result['status'],
				'aprov'           => $this->url->link('seller/seller/aprov', 'token=' . $this->session->data['token'] . '&vyvodid=' . $result['id'] , true),
				'status' => $stat
				
			);
		}

		$data['heading_title'] = 'Заявки на вывод';
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('seller/vyvod_list', $data));
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			if (!$result['approved']) {
				$approve = $this->url->link('seller/seller/approve', 'token=' . $this->session->data['token'] . '&seller_id=' . $result['seller_id'] . $url, true);
			} else {
				$approve = '';
			}

			$login_info = $this->model_seller_seller->getTotalLoginAttempts($result['email']);

			if ($login_info && $login_info['total'] >= $this->config->get('config_login_attempts')) {
				$unlock = $this->url->link('seller/seller/unlock', 'token=' . $this->session->data['token'] . '&email=' . $result['email'] . $url, true);
			} else {
				$unlock = '';
			}
			...
			$data['sellers'][] = array(
				...
				'approve'        => $approve,
				'unlock'         => $unlock,
				'edit'           => $this->url->link('seller/seller/edit', 'token=' . $this->session->data['token'] . '&seller_id=' . $result['seller_id'] . $url, true)
			);
		}
		...
		$data['sort_status'] = $this->url->link('seller/seller', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, true);
...
if (isset($this->error['keyword_error'])) {
			$data['error_keyword'] = $this->error['keyword_error'];
		} else {
			$data['error_keyword'] = '';
		}
		...
		if (isset($this->error['seller_field'])) {
			$data['error_seller_field'] = $this->error['seller_field'];
		} else {
...
if (isset($this->request->post['keyword'])) {
		...
		// Custom Fields
		$this->load->model('seller/seller_field');

		$data['seller_fields'] = array();
		...
		if (isset($this->request->post['seller_field'])) {
			$data['account_seller_field'] = $this->request->post['seller_field'];
		} elseif (!empty($seller_info)) {
			$data['account_seller_field'] = json_decode($seller_info['seller_field'], true);
		} else {
			$data['account_seller_field'] = array();
		}
	...
	if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['seller_id']) && $url_alias_info['query'] != 'manufacturer_id=' . $this->request->get['seller_id']) {
				$this->error['keyword_error'] = sprintf($this->language->get('error_keyword'));
			}

			if ($url_alias_info && !isset($this->request->get['seller_id'])) {
				$this->error['keyword_error'] = sprintf($this->language->get('error_keyword'));
			}
		}
		...
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {
				...
				foreach ($seller_fields as $seller_field) {
					if (($seller_field['location'] == 'address') && $seller_field['required'] && empty($value['seller_field'][$seller_field['seller_field_id']])) {
						$this->error['address'][$key]['seller_field'][$seller_field['seller_field_id']] = sprintf($this->language->get('error_seller_field'), $seller_field['name']);
					} elseif (($seller_field['location'] == 'address') && ($seller_field['type'] == 'text') && !empty($seller_field['validation']) && !filter_var($value['seller_field'][$seller_field['seller_field_id']], FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $seller_field['validation'])))) {
						$this->error['address'][$key]['seller_field'][$seller_field['seller_field_id']] = sprintf($this->language->get('error_seller_field'), $seller_field['name']);
                    }
				}
			}
	...
	public function ip() {
		...
		foreach ($results as $result) {
			$data['ips'][] = array(
				...
				'filter_ip'  => $this->url->link('seller/seller', 'token=' . $this->session->data['token'] . '&filter_ip=' . $result['ip'], true)
			);
		}
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('seller/seller/ip', 'token=' . $this->session->data['token'] . '&seller_id=' . $this->request->get['seller_id'] . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('seller/seller_ip', $data));
	}
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					...
					'telephone'         => $result['telephone'],
					'fax'               => $result['fax'],
					'seller_field'      => json_decode($result['seller_field'], true),
					'address'           => $this->model_seller_seller->getAddresses($result['seller_id'])
				);
			}
		}
