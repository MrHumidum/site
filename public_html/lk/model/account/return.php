...
class ModelAccountReturn extends Model {
	public function addReturn($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "vyvod` SET user_id = '" . (int)$this->customer->getId() . "', summ = '" . $this->db->escape($data['summ']) . "', rek = '" . $this->db->escape($data['rek']) . "', bank = '" . $this->db->escape($data['bank']) . "', status = '0', date = NOW()");
	...
	public function getReturns($start = 0, $limit = 20) {
		...
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "vyvod`  WHERE user_id = '" . $this->customer->getId() . "'  ORDER BY id DESC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}

	public function getTotalReturns() {
		$query = $this->db->query("SELECT COUNT(*) AS total  FROM `" . DB_PREFIX . "vyvod`  WHERE user_id = '" . $this->customer->getId() . "' ");
