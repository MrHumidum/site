...
class ControllersellerCustomField extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('seller/custom_field');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('seller/custom_field');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_seller_custom_field->addCustomField($this->request->post);
			...
			$this->response->redirect($this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_seller_custom_field->editCustomField($this->request->get['custom_field_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $custom_field_id) {
				$this->model_seller_custom_field->deleteCustomField($custom_field_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('seller/custom_field/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('seller/custom_field/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$custom_field_total = $this->model_seller_custom_field->getTotalCustomFields();

		$results = $this->model_seller_custom_field->getCustomFields($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['custom_fields'][] = array(
				...
				'edit'            => $this->url->link('seller/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $result['custom_field_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . '&sort=cfd.name' . $url, true);
		$data['sort_location'] = $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.location' . $url, true);
		$data['sort_type'] = $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.type' . $url, true);
		$data['sort_status'] = $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.status' . $url, true);
		$data['sort_sort_order'] = $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('seller/custom_field_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['custom_field_id'])) {
			$data['action'] = $this->url->link('seller/custom_field/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('seller/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $this->request->get['custom_field_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('seller/custom_field', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['custom_field_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$custom_field_info = $this->model_seller_custom_field->getCustomField($this->request->get['custom_field_id']);
		}
		...
		if (isset($this->request->post['custom_field_description'])) {
			...
			$data['custom_field_description'] = $this->model_seller_custom_field->getCustomFieldDescriptions($this->request->get['custom_field_id']);
		} else {
		...
		if (isset($this->request->post['custom_field_value'])) {
			...
			$custom_field_values = $this->model_seller_custom_field->getCustomFieldValueDescriptions($this->request->get['custom_field_id']);
		} else {
		...
		if (isset($this->request->post['custom_field_seller_group'])) {
			$custom_field_seller_groups = $this->request->post['custom_field_seller_group'];
		} elseif (isset($this->request->get['custom_field_id'])) {
			$custom_field_seller_groups = $this->model_seller_custom_field->getCustomFieldsellerGroups($this->request->get['custom_field_id']);
		} else {
			$custom_field_seller_groups = array();
		}

		$data['custom_field_seller_group'] = array();

		foreach ($custom_field_seller_groups as $custom_field_seller_group) {
			$data['custom_field_seller_group'][] = $custom_field_seller_group['seller_group_id'];
		}

		$data['custom_field_required'] = array();

		foreach ($custom_field_seller_groups as $custom_field_seller_group) {
			if ($custom_field_seller_group['required']) {
				$data['custom_field_required'][] = $custom_field_seller_group['seller_group_id'];
			}
		}
		...
		$this->response->setOutput($this->load->view('seller/custom_field_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'seller/custom_field')) {
