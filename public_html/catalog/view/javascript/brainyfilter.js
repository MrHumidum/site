        ...
        updateProductList: function(html, bfData) {
            var $gridBtn = $('#grid-view').clone(true), 
                ...
                $prodCont = $html.find(BrainyFilter.selectors.container),
                 $prodCount = $html.find(BrainyFilter.selectors.resultscount),
                $paginCont = $html.find(BrainyFilter.selectors.paginator),
                $emptyMsg = $('<p/>', {'class': 'bf-empty-product-list-msg'}).html(bfLang.empty_list)
                ;
$('#totalcount').html($prodCount.html());
$('#totalcountmob').html($prodCount.html());
            if ($prodCont.size()) {
            ...
            $.fn.bfparam = function(bfparam){
                if (bfparam !== '') {
                    this.each(function(){
                        ...
                        url = $(this).attr(attr);
                        url = url.replace(/[\?\&](bfilter|autor_id|search|category_id)\=[^\&]+/g, '');
                        url += /\?/.test(url) ? '&' : '?';
                        url += bfilter;
                        $this.attr(attr, url);
                    });
        ...
        prepareFilterData: function(full, route) {
            ...
            query = window.location.search.replace(/[\&\?]+((route)|(path)|(page)|(bfilter)|(search)|(category_id)|(sub_category)|(description)|(autor_id))[^&]+/g, '');
            ...
            if (route) {
                ...
                form = form.replace(/[\&](route|path|autor_id)=[^\&]+/g, '').replace(/[\&]+$/, '');
                if (nPath !== '') {
    ...
    //        var searchFilters = {};
            if (params.length) {
                for (var i = 0; i < params.length; i ++) {
                    ...
                    } else if (name === 'autor_id') {
                        brandFilter = 'autor_id=' + value;
                    }
                }
            }
